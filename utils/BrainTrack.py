...
class BrainTrack(BrainDetect):
    ...
    def set_tracking(
        self, HORIZONTAL=False, VERTICAL=True, DISTANCE=True, ROTATION=True
    ):
        """
    ...
    def __worker(self):
        ...
        if self.frame is not None and self.cycle_counter % self.cycle_activation == 0:
            ...
            if det is not None and len(det) > 0:
                ...
                x_control_effort, x_error = self.pid_x.control_effort(
                    self.cx, x_feedback
                )
                y_control_effort, y_error = self.pid_y.control_effort(
                    self.cy, y_feedback
                )
                z_control_effort, z_error = self.pid_z.control_effort_depth(
                    self.cz, z_feedback
                )
                # print("---------------")
    ...
    def draw_detections(self, img):
        ...
        if img is not None:
            ...
            if self.det is not None:
                ...
                cv2.circle(
                    img,
                    (int(self.cx * self.w), int(self.cy * self.h)),
                    4,
                    [0, 255, 0],
                    1,
                )
